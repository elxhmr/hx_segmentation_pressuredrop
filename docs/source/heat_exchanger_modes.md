...
### Geometry-driven configuration

Condenser and evaporator instances can now be built directly from an HX
geometry definition instead of a raw area. Passing `geometry=load_geometry(...)`
populates the stored outer/inner areas, characteristic length, and hydraulic
diameter for later use, and the outer area is used as `A` when an explicit area
is not provided. If no `ratio_outer_to_inner_area` is supplied, the ratio is
derived from the loaded geometry to keep NTU calculations consistent with the
physical surface areas.【F:vclibpy/components/heat_exchangers/hx_model/__init__.py†L1-L14】【F:vclibpy/components/heat_exchangers/heat_exchanger.py†L55-L87】【F:vclibpy/components/heat_exchangers/ntu.py†L16-L39】
