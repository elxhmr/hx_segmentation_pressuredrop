...
class MovingBoundaryNTU(BasicNTU, abc.ABC):
    ...
    def _calc_segment_dp(self, segment_area: float, state_in, state_out) -> float:
        ...
        if not self.dp_model or self.A <= 0:
            return 0.0

        transport_properties = self.med_prop.calc_mean_transport_properties(state_in, state_out)
        base_dp = self.dp_model.calc(transport_properties=transport_properties, m_flow=self.m_flow)
        if base_dp <= 0:
            return 0.0
    ...
    def _segment_phase(
        ...
        for seg_index in range(n_segments):
            ...
            dp_segment = self._calc_segment_dp(segment_area, state_in, state_out)
